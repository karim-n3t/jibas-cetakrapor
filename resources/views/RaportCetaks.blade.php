        ...
        <tbody>
            <tr>
                ...
                <td width="25%">Sragen,
                    ...
                    {{ $kelas }}<br><br><br><br><b>{{ $walikelas }}</b><br>NIP. -</td>
            </tr>
        </tbody>
    </table><br>
            ...
            <div class="card ">
                ...
                <div class="card-body">
                    <table class="table-borderless siswa" style="color:black" width="100%">
                        <tbody>
                            <tr>
                                <td width="20%">Departemen<br>Tahun ajaran<br>Semester<br>Tanggal presensi
                                </td>
                                <td width="40%">: {{ $departemen ?? '' }}<br>: {{ $tahunajaran ?? '' }}<br>:
                                    {{ $semester }}<br>: {{ $tanggalawal }} sampai {{ $tanggalakhir }}</td>
                                <td>
                                    <button id="cetak" class="btn btn-sm btn-outline-info"
                                        onclick="printDiv('printCover')"><i class="material-icons">insert_drive_file</i>
                                        Cover</button>
                                    <button id="cetak" class="btn btn-sm btn-outline-success"
                                        onclick="printDiv('printBiodata')"><i class="material-icons">person</i>
                                        Biodata</button>
                                    <button id="cetak" class="btn btn-sm btn-outline-warning"
                                        onclick="printDiv('printRaport')"><i class="material-icons">print</i>
                                        Raport</button>
                                </td>
                            </tr>
                        </tbody>
            ...
            <div class="card">
                ...
                <div class="card-body d-none d-print-block print" id="printCover">
                    <div class="text-center">
                        ...
                        <h3 class="text-uppercase font-weight-bold">Raport Peserta Didik<br> Sekolah
                            Menengah Kejuruan<br>(SMK)</h3>
                        ...
                        <h2 class="font-weight-bold text-lowercase text-capitalize">{{ $s->nama }}</h2><br
                            style="display: block;content: '';margin: 100px 0;">
                        <p style="font-size:x-large;">NIS / NISN :</p>
                        <h2 class="font-weight-bold">{{$s->nis}} / {{$s->nisn}}</h2><br
                            style="display: block;content: '';margin: 250px 0;">
                        <h3 class="text-uppercase font-weight-bold">Kementrian Pendidikan dan Kebudayaan
                ...
                <div class="card-body d-none d-print-block print" id="printBiodata">
                    <div>
                        <h3 class="text-uppercase font-weight-bold text-center">Keterangan Tentang Diri
                            Peserta Didik</h3>
                        <br style="display: block;content: '';margin: 100px 0;">
                            ...
                            <tbody style="font-size:large; line-height:2;">
                                <tr>
                                    ...
                                    <td class="pl-3">
                                        ...
                                        <br>
                                        X<br>
                                        @if ($sis->tahunmasuk == '2018') 16 Juli 2018 @elseif
                                        ($sis->tahunmasuk == '2019') 15 Juli 2019 @endif<br>
                                        <br>
                                        {{$sis->namaayah}}<br>
                                        {{$sis->namaibu}}<br>
                                        {{$sis->alamatortu}}<br>
                        ...
                        <table class="table-borderless">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="pl-5 pr-5 text-center border border-dark">Pas Foto <br> 3 x 4
                                    </td>
                                    <td class="pl-5 pr-5"></td>
                                    <td class="pl-5 font-height">
                                        <h4>Sragen, @if ($sis->tahunmasuk == '2018') 16 Juli 2018 @elseif
                                            ($sis->tahunmasuk == '2019') 15 Juli 2019 @endif<br> Kepala
                                            Sekolah
                ...
                <div class="card-body print" id="printRaport">
                        ...
                        <table class="table">
                            <thead>
                                ...
                                <th class="text-center" data-toggle="tooltip"
                                    title="{{ $a->keterangan }} ({{ $a->dasarpenilaian }})">
                                    {{ $a->keterangan }}</th>
                                @endforeach
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="{{ $colspan }}" data-toggle="tooltip"
                                        title="{{ $k->idkelompok }} - {{ $k->kode }}. {{ $k->kelompok }}">
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td colspan="{{ $colspan }}" data-toggle="tooltip"
                                        title="{{ $p->idpelajaran }} - {{ $p->nama }}">{{ $p->nama }}</td>
                                    @if (isset($aspek))
                                    @php ($nilaiakhir = array()) @endphp
                                    @foreach ($aspek as $a)
                                    @php ($nilai = (collect($nilaisiswa)->where('idpelajaran',
                                    $p->idpelajaran)->where('dasarpenilaian', $a->dasarpenilaian)->take(1)))
                                    @endphp
                                    @if ($nilai->count() > 0)
                                    @foreach ($nilai as $n)
                                    ...
                                    <td class="text-center" data-toggle="tooltip"
                                        title="{{ $n->replid ?? '' }} - {{ $p->nama }} ({{ $a->keterangan }}) : {{ $n->nilaiangka ?? '' }} ({{ $n->nilaihuruf ?? '' }})">
                                        {{ number_format($n->nilaiangka) ?? '-' }}</td>
                                    @endforeach
                                    @else
                                    ...
                                    @if (isset($nipguru))
                                    <td class="text-center" data-toggle="tooltip"
                                        title="@foreach ($nilaiakhir as $na) {{ $na }}, @endforeach {{ $ratanilai ?? '' }}">
                                        {{ number_format($ratanilai) ?? '' }}</td>
                    ...
                    @foreach (collect($grade)->where('idpelajaran', $idpelajaran)->where('nipguru', $nipguru) as $g) <b>{{ $g->grade }}</b> : {{ $g->nmin }} - {{ $g->nmax }} <br/> @endforeach">
                                        @foreach (collect($grade)->where('idpelajaran',
                                        $idpelajaran)->where('nipguru', $nipguru)->where('nmin', '<=',
                                            number_format($ratanilai) ?? '' )->where('nmax', '>=',
                                            number_format($ratanilai) ?? '') as $g) {{ $g->grade }}
                                            @endforeach</td>
                                    @endif
                                </tr>@php ($i++) @endphp
                                @endforeach
                        ...
                        <table class="table" height="120px">
                            <tbody>
                                <tr>
                                    <td>
                                        @if (isset($nilaisiswa))
                                        Ananda perlu meningkatkan kompetensi pengetahuan @foreach
                                        (collect($nilaisiswa)->where('dasarpenilaian',
                                        'PNGT')->sortBy('nilaiangka')->take(3) as $n) {{ $n->nama }},
                                        @endforeach sebagai bekal pembelajaran kompetensi kejuruan di
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    @endphp
                                    @if (empty($nkomentar) || !isset($nkomentar) || $nkomentar == '' ||
                                    collect($nkomentar)->isEmpty())
                                    ...
                                    <td class="text-center"></td>
                                    @else
                                    {!! str_replace("<ol>", "", (str_replace("</ol>", "",
                                    (str_replace("li>", "td>", $komentar->komentar))))) !!}
                                    @endif
                    ...
                    <br/>
                    @foreach (collect($grades)->where('idpelajaran', $idpelajaran)->where('nipguru', $nippkl) as $g) <b>{{ $g->grade }}</b> : {{ $g->nmin }} - {{ $g->nmax }} <br/> @endforeach">
                                        @foreach (collect($grades)->where('idpelajaran',
                                        $idpelajaran)->where('nipguru', $nippkl)->where('nmin', '<=',
                                            number_format($ratanilai) ?? '' )->where('nmax', '>=',
                                            number_format($ratanilai) ?? '') as $g) {{ $g->grade }}
                                            @endforeach</td>
                                    @endif
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td data-toggle="tooltip"
                                        title="{{ $n->replid ?? '' }} - {{ $p->nama }} ({{ $a->keterangan }}) : {{ $n->nilaiangka ?? '' }} ({{ $n->nilaihuruf ?? '' }})">
                                        Melaksanakan kegiatan {{ $p->nama }} dengan @if ($n->nilaihuruf ==
                                        'A') Sangat baik @elseif ($n->nilaihuruf == 'B') Baik @elseif
                                        ($n->nilaihuruf == 'C') Cukup baik @elseif ($n->nilaihuruf == 'D')
                                        Kurang baik @else Baik @endif </td>
                                    @endforeach
                                    @else
                    ...
                    <div class="table-sm">
                        <table class="table" style="width: 50%" data-toggle="tooltip" data-html="true"
                            title="Presensi siswa dari tanggal<br/>{{ $tanggalawal }} <br/>sampai<br/> {{ $tanggalakhir }}">
                            <tbody>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            <tbody>
                                <td>Naik / <del>Tidak naik</del> ke kelas
                                    <b>{{ str_replace("X", "XI", $kelas) }}</b></td>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="text-center" data-toggle="tooltip"
                                        title="{{ $p->idpelajaran }} - {{ $p->nama }}">{{ $p->nama }}</td>
                                    @foreach (collect($nilaisiswa)->where('idpelajaran', $p->idpelajaran) as
                                    $n)
                                    <td data-toggle="tooltip"
                                        title="{{ $n->replid ?? '' }} - {{ $p->nama }} ({{ $a->keterangan ?? '' }}) : {{ $n->nilaiangka ?? '' }} ({{ $n->nilaihuruf ?? '' }})">
                                        @if (($p->nama == 'Integritas') AND ($n->nilaihuruf == 'A')) Ananda
                                        selalu menjunjung tinggi nilai kesetiaan, keteladanan, anti korupsi,
                                        tidak menyalahgunakan pembayaran sekolah, membayar tepat waktu serta
                                        selalu menghargai harkat martabat manusia
                                        @elseif (($p->nama == 'Integritas') AND ($n->nilaihuruf == 'B'))
                                        Ananda sangat menghargai dan setia pada teman, selalu memberi contoh
                                        perilaku yang baik pada teman dan tidak menyalahgunakan keuangan
                                        sekolah
                                        @elseif (($p->nama == 'Integritas') AND ($n->nilaihuruf == 'C'))
                                        Ananda selalu setia dan memberikan keteladanan perilaku yang baik
                                        pada teman sekelas
                                        @elseif (($p->nama == 'Religius') AND ($n->nilaihuruf == 'A'))
                                        Ananda menunjukkan ketagwaan yang tinggi selalu taat dan tertib
                                        menjalankan ibadah dengan sholat berjamaah di masjid,menjauhi
                                        larangan agama serta toleran pada penganut agama yang berbeda
                                        @elseif (($p->nama == 'Religius') AND ($n->nilaihuruf == 'B'))
                                        Ananda selalu taat dan tertib menjalankan ibadah setiap hari serta
                                        selalu menjaga diri dari perbuatan yang di larang agama
                                        ...
                                        Ananda selalu menunjukkan sikap disiplin yang tinggi, cinta damai,
                                        taat pada peratauran hukum dan tidak pernah melakukan pelanggaran
                                        ringan, sedang berat atau berat sesuai tata tertib yang menjadi
                                        aturan sekolah
                                        @elseif (($p->nama == 'Nasionalis') AND ($n->nilaihuruf == 'B'))
                                        Ananda selalu menunjukkan sikap disiplin yang tinggi, mentaati
                                        peraturan lalu lintas dan tata tertib sekolah serta mempunyai rasa
                                        cinta damai yang tinggi
                                        @elseif (($p->nama == 'Nasionalis') AND ($n->nilaihuruf == 'C'))
                                        Ananda selalu berdisiplin dan taat pada tata tertib sekolah serta
                                        aktif mengikuti kegiatan upacara
                                        @elseif (($p->nama == 'Mandiri') AND ($n->nilaihuruf == 'A')) Ananda
                                        selalu menunjukkan sikap kerja keras yang tinggi, tidak gampang
                                        mengeluh selalu bersemangat, kreatif, memiliki daya juang serta
                                        berwawasan dan teknologi dalam setiap kegiatan pembelajaran di
                                        sekolah
                                        @elseif (($p->nama == 'Mandiri') AND ($n->nilaihuruf == 'B')) Ananda
                                        selalu menunjukkan sikap kerja keras yang tinggi, kreatif, memiliki
                                        daya juang yang kuat dalam setiap kegiatan
                                        @elseif (($p->nama == 'Mandiri') AND ($n->nilaihuruf == 'C')) Ananda
                                        selalu menunjukkan sikap kerja keras yang tinggi, kreatif dalam
                                        setiap kegiatan serta suka membantu teman di sekolah.
                                        @elseif (($p->nama == 'Gotong Royong') AND ($n->nilaihuruf == 'A'))
                                        Ananda selalu menunjukkan sikap kerelawanan, saling tolong menolong,
                                        tidak membeda bedakan teman, serta selalu bermusyawarah dalam
                                        menyelesaiakan permasalahan
                                        @elseif (($p->nama == 'Gotong Royong') AND ($n->nilaihuruf == 'B'))
                                        Ananda selalu menunjukkan sikap suka tolong menolong, tidak membeda
                                        bedakan teman, serta selalu bermusyawarah dalam menyelesaikan
                                        permasalahan
                                        @elseif (($p->nama == 'Gotong Royong') AND ($n->nilaihuruf == 'C'))
                                        Ananda selalu menunjukkan sikap suka tolong menolong, serta selalu
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            <tbody>
                                <tr>
                                    <td>Ananda perlu meningkatkan kompetensi pengetahuan sebagai bekal
                                        pembelajaran kompetensi kejuruan di semester selanjutnya.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div><br><br><br>
